...
from newiktest import CCDSolver, Render
import torch
import numpy as np

from zed_utilities import body_34_parts, body_34_tree, body_34_tpose, Quaternion, Position, ForwardKinematics
...
class Nothing:
    ...
    def __str__(self):
        return "Nothing to see here (%s)"%(self.name)
...
modes = {'pre-pass rot:' : RootTrans,
         ...
         'pre-pass fullrots' : Nothing,
         'post-pass fullrots' : Nothing
         }
...
p_ipos = ldata['pre-pass rot:'].pos
p_poses = fkn.propagate(q_rots, p_ipos)
problem = ldata['Effector position:']

iksolver = CCDSolver(body_34_parts, body_34_tree, 'PELVIS', q_rots, p_poses, rootpos = p_ipos)

test_ccd = iksolver.CCD_pass(problem.effector, problem.pivot_bone, problem.end_bone, piv_to_end = True)
new_poses = fkn.propagate(test_ccd, p_ipos)

actual_poses = ldata['post-pass keypoints'].vecs
#actual_recentred = actual_poses = [p - actual_poses[0] for p in actual_poses]
#rlist = [p_poses, new_poses, actual_recentred]
rlist = [p_poses, new_poses, actual_poses]
